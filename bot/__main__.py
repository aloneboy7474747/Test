...
async def main():
    # Load side channel from DB if set
    try:
        from bot.core.database import db
        sc = await db.get_setting("side_channel")
        if sc:
            Var.SIDE_CHANNEL = int(sc)
            LOGS.info(f"Loaded SIDE_CHANNEL from DB: {Var.SIDE_CHANNEL}")
    except Exception as _e:
        LOGS.warning(f"Could not load side_channel from DB: {_e}")
