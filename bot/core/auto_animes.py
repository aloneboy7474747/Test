...
async def mirror_to_main_channel(post_msg, photo_url, caption, channel_to_use):
    try:
        ...
        # ── Build link button to side channel post ───────────────────────────
        buttons = None
        try:
            if post_msg:
                ...
                # Always include side channel join button if it's a private channel
        ...
        main_post = None
        try:
            # ── Post to MAIN channel (photo + caption + link button) ──────────
            # Files are always in side/anime channel; main channel just gets the post
...
async def _backfill_previous_episodes(name: str, aniInfo, current_ep: int, sample_url):
    """
    When a new ongoing anime is first detected, fetch all previous episodes
    from RSS feeds and process them so nothing is missed.
    """
    try:
        await rep.report(f"Starting backfill for {name} — fetching episodes 1 to {current_ep - 1}", "info")

        anime_title_clean = extract_base_anime_name(name)

        for qual, rss_url in Var.RSS_LOW_END.items():
            try:
                ...
                for item in feed.entries:
                    ...
                    item_link  = getattr(item, 'link',  '') or ''

                    # Skip if title doesn't match this anime
                    if anime_title_clean.lower() not in item_title.lower():
                        continue

                    # Skip current or future episodes
                    ep_match = re.search(r'[-_ ](\d{2,3})[-_ \[]', item_title)
                    if ep_match:
                    ...
                    if ani_data and ani_data.get(ep_str):
                        ep_quals = ani_data[ep_str]
                        if all(ep_quals.get(q) for q in ep_quals):
                            continue  # already uploaded

                    await rep.report(f"Backfilling episode {ep_num} of {name}: {item_title}", "info")
                    await asleep(3)  # avoid flooding
                    await process_low_end(item_title, item_link, force=False)

            except Exception as e:
                await rep.report(f"Backfill RSS error for {name} qual={qual}: {e}", "warning")
                continue

        await rep.report(f"Backfill complete for {name}", "info")

    except Exception as e:
        await rep.report(f"Backfill failed for {name}: {e}", "error")
...
async def process_low_end(name, torrent_url, force=False):
    try:
        ...
        is_new_anime = ani_id not in ani_cache['ongoing']
        if is_new_anime:
        ...
        # ── Auto-fetch previous episodes for newly detected ongoing anime ─────
        if is_new_anime and not force:
            try:
                total_eps = aniInfo.adata.get('episodes') or 0
                current_ep = int(ep_no) if ep_no and str(ep_no).isdigit() else 1
                if total_eps and current_ep > 1:
                    await rep.report(
                        f"New ongoing anime detected: {name}\n"
                        f"Current episode: {current_ep}, Total: {total_eps}\n"
                        f"Will attempt to fetch previous {current_ep - 1} episodes from RSS...",
                        "info"
                    )
                    # Trigger backfill via RSS for previous episodes
                    asyncio.ensure_future(
                        _backfill_previous_episodes(name, aniInfo, current_ep, torrent_url)
                    )
            except Exception as _e:
                await rep.report(f"Failed to schedule backfill for {name}: {_e}", "warning")
...
async def get_or_create_anime_channel(name: str, aniInfo):
    try:
        ...
        channel_creation = await db.get_channel_creation()
        if not channel_creation or not Var.USER_SESSION:
            # Check per-anime side channel set via /addanime name - main - side
            try:
                per_anime_side = await db.get_setting(f"side_channel_{normalized_name}")
                if per_anime_side:
                    await rep.report(f"Using per-anime side channel {per_anime_side} for {base_anime_name}", "info")
                    return int(per_anime_side)
            except Exception:
                pass
            # Global side channel fallback
            side_ch = getattr(Var, 'SIDE_CHANNEL', 0)
            if side_ch:
                await rep.report(f"Using global SIDE_CHANNEL {side_ch} for {base_anime_name}", "info")
                return side_ch
        ...
        if new_channel:
            ...
            side_ch = getattr(Var, 'SIDE_CHANNEL', 0)
            if side_ch:
                await rep.report(f"Channel creation failed — using SIDE_CHANNEL {side_ch}", "warning")
                return side_ch
        ...
        side_ch = getattr(Var, 'SIDE_CHANNEL', 0)
        return side_ch if side_ch else Var.MAIN_CHANNEL
