...
async def set_anime_channel(client, message: Message):
    """
    Usage:
      /addanime <name> - <main_channel>
      /addanime <name> - <main_channel> - <side_channel>

    With side_channel: quality files upload to side_channel,
    main_channel gets the post with a download link button.
    """
    try:
        ...
        if " - " not in text:
            return await message.reply(
                "<blockquote><b>Usage:</b>\n"
                "/addanime anime name - @channel\n"
                "/addanime anime name - @main_channel - @side_channel\n\n"
                "With side channel: quality files go to side channel, "
                "main channel gets post with download button.</blockquote>"
            )

        parts = [p.strip() for p in text.split(" - ")]
        anime_name = parts[0]
        channel_raw = parts[1] if len(parts) > 1 else ""
        side_raw = parts[2] if len(parts) > 2 else ""

        if not anime_name or not channel_raw:
            return await message.reply("<blockquote><b>Please provide anime name and channel.</b></blockquote>")
        ...
        final_channel = parse_ch(channel_raw)
        final_side = parse_ch(side_raw)

        if final_channel is None:
            return await message.reply("<blockquote><b>Invalid main channel. Use @username or -100ID.</b></blockquote>")

        # Validate channels
        try:
            chat = await bot.get_chat(final_channel)
            main_title = chat.title
        except Exception as e:
            return await message.reply(f"<blockquote><b>Cannot access main channel: {e}</b></blockquote>")

        side_title = None
        if final_side:
            try:
                side_chat = await bot.get_chat(final_side)
                side_title = side_chat.title
            except Exception as e:
                return await message.reply(f"<blockquote><b>Cannot access side channel: {e}</b></blockquote>")

        possible_names = await get_all_possible_anime_names(anime_name)
        if not possible_names:
            possible_names = [anime_name.lower()]

        for name in possible_names:
            await db.add_anime_channel_mapping(name, final_channel)
            if final_side:
                await db.set_setting(f"side_channel_{name}", final_side)

        reply = (
            f"<blockquote><b>✅ Mapped: {anime_name}</b>\n"
            f"📺 Main channel: {main_title} (<code>{final_channel}</code>)\n"
        )
        if final_side:
            reply += f"📦 Side channel: {side_title} (<code>{final_side}</code>)\n"
        reply += f"🔀 Variants mapped: <code>{len(possible_names)}</code></blockquote>"

        await message.reply(reply)

    except Exception as e:
        print(f"Error in /addanime: {e}")
...
# ── 1. /animeinfo — Get AniList info for any anime ──────────────────
@bot.on_message(filters.command("animeinfo") & filters.user(Var.ADMINS))
@new_task
async def anime_info_cmd(client, message: Message):
    """Get detailed AniList info for any anime."""
    name = message.text[len("/animeinfo"):].strip()
    if not name:
        return await message.reply("<blockquote><b>Usage: /animeinfo anime name</b></blockquote>")
    msg = await message.reply("<blockquote>🔍 Fetching from AniList...</blockquote>")
    try:
        ...
        d = te.adata
        if not d:
            return await msg.edit("<blockquote><b>❌ Anime not found on AniList.</b></blockquote>")
        title = d.get('title', {})
        en = title.get('english') or title.get('romaji') or name
        genres = ', '.join(d.get('genres', [])) or 'N/A'
        status = d.get('status', 'N/A')
        eps = d.get('episodes') or '?'
        score = d.get('averageScore') or 'N/A'
        year = (d.get('startDate') or {}).get('year') or 'N/A'
        desc = d.get('description') or 'No description.'
        import re
        desc = re.sub(r'<[^>]+>', '', desc)[:300] + '...'
        cover = (d.get('coverImage') or {}).get('large') or ''
        text = (
            f"<blockquote><b>📺 {en}</b></blockquote>\n"
            f"<blockquote>"
            f"• <b>Status:</b> {status}\n"
            f"• <b>Episodes:</b> {eps}\n"
        ...
        await msg.edit(f"<blockquote><b>Error: {e}</b></blockquote>")


# ── 2. /searchanime — Search AniList for top results ───────────────
@bot.on_message(filters.command("searchanime") & filters.user(Var.ADMINS))
@new_task
async def search_anime_cmd(client, message: Message):
    """Search AniList and show top 5 results."""
    name = message.text[len("/searchanime"):].strip()
    if not name:
        return await message.reply("<blockquote><b>Usage: /searchanime anime name</b></blockquote>")
    msg = await message.reply("<blockquote>🔍 Searching AniList...</blockquote>")
    try:
        ...
        if not results:
            return await msg.edit("<blockquote><b>No results found.</b></blockquote>")
        text = f"<blockquote><b>🔍 Search results for: {name}</b></blockquote>\n\n"
        for i, r in enumerate(results, 1):
            t = r.get('title', {})
            en = t.get('english') or t.get('romaji') or 'Unknown'
            status = r.get('status', '?')
            eps = r.get('episodes') or '?'
            score = r.get('averageScore') or '?'
            year = (r.get('startDate') or {}).get('year') or '?'
            text += (
                f"<b>{i}. {en}</b>\n"
                f"   Status: {status} | Eps: {eps} | Score: {score} | Year: {year}\n\n"
            )
        await msg.edit(text)
    except Exception as e:
        await msg.edit(f"<blockquote><b>Error: {e}</b></blockquote>")
...
# ── 3. /broadcast — Broadcast to all users ─────────────────────────
@bot.on_message(filters.command("broadcast") & filters.user(Var.ADMINS))
@new_task
async def broadcast_cmd(client, message: Message):
    """Reply to a message to broadcast it to all bot users."""
    if not message.reply_to_message:
        return await message.reply("<blockquote><b>Reply to a message to broadcast it.</b></blockquote>")
    from asyncio import sleep as asl
    from pyrogram.errors import FloodWait, UserIsBlocked, InputUserDeactivated
    users = await db.get_all_users()
    total = len(users)
    status = await message.reply(f"<blockquote>📢 Broadcasting to {total} users...</blockquote>")
    ...
    await status.edit(
        ...
        f"Total: {total} | ✅ {success} | ❌ {failed} | 🚫 {blocked}</blockquote>"
    )


# ── 4. /ping — Bot response time ────────────────────────────────────
...
async def ping_cmd(client, message: Message):
    ...
    await msg.edit(f"<blockquote>🏓 <b>Pong!</b>\nResponse: <code>{ms}ms</code></blockquote>")


# ── 5. /queuestatus — Show current encode queue ─────────────────────
@bot.on_message(filters.command("queuestatus") & filters.user(Var.ADMINS))
@new_task
async def queue_status_cmd(client, message: Message):
    from bot import ffQueue, ff_queued
    size = ffQueue.qsize()
    queued = list(ff_queued.keys()) if ff_queued else []
    text = (
        f"<blockquote><b>⚙️ Encode Queue Status</b>\n\n"
        f"Queue size: <code>{size}</code>\n"
        f"Active tasks: <code>{len(queued)}</code>\n"
    )
    if queued:
        text += "\n<b>Active:</b>\n"
        for k in queued[:10]:
...
# ── 6. /viewbanner — View anime banner ─────────────────────────────
@bot.on_message(filters.command("viewbanner") & filters.user(Var.ADMINS))
@new_task
async def view_anime_banner_cmd(client, message: Message):
    name = message.text[len("/viewbanner"):].strip()
    if not name:
        return await message.reply("<blockquote><b>Usage: /viewbanner anime name</b></blockquote>")
    all_banners = await db.list_anime_banners()
    for key, url in all_banners:
        if name.lower() in key.lower():
            return await message.reply_photo(photo=url, caption=f"<b>Banner for: {key}</b>")
    await message.reply(f"<blockquote><b>No banner found for: {name}</b></blockquote>")


# ── 7. /delallbanners — Delete all anime banners ───────────────────
@bot.on_message(filters.command("delallbanners") & filters.user(Var.ADMINS))
@new_task
async def del_all_banners_cmd(client, message: Message):
    try:
        all_banners = await db.list_anime_banners()
        for key, _ in all_banners:
            await db.del_anime_banner(key)
        await message.reply(f"<blockquote><b>✅ Deleted all {len(all_banners)} banners.</b></blockquote>")
    except Exception as e:
        await message.reply(f"<blockquote><b>Error: {e}</b></blockquote>")


# ── 8. /delallanimes — Delete all anime mappings ──────────────────
@bot.on_message(filters.command("delallanimes") & filters.user(Var.ADMINS))
@new_task
async def del_all_animes_cmd(client, message: Message):
    try:
        await db.remove_all_anime_channel_mappings()
        await message.reply("<blockquote><b>✅ All anime channel mappings removed.</b></blockquote>")
    except Exception as e:
        await message.reply(f"<blockquote><b>Error: {e}</b></blockquote>")


# ── 9. /addfsub /delfsub /listfsubs — Force sub management ─────────
@bot.on_message(filters.command("addfsub") & filters.user(Var.ADMINS))
@new_task
async def add_fsub_cmd(client, message: Message):
    args = message.text.split(None, 1)
    if len(args) < 2:
        return await message.reply("<blockquote><b>Usage: /addfsub channel_id</b></blockquote>")
    try:
        cid = int(args[1].strip())
        chat = await bot.get_chat(cid)
        await db.add_fsub(cid)
        await update_fsub_chats_var()
        await message.reply(f"<blockquote><b>✅ Added FSub: {chat.title} ({cid})</b></blockquote>")
    except Exception as e:
        await message.reply(f"<blockquote><b>Error: {e}</b></blockquote>")
...
async def del_fsub_cmd(client, message: Message):
    args = message.text.split(None, 1)
    if len(args) < 2:
        return await message.reply("<blockquote><b>Usage: /delfsub channel_id</b></blockquote>")
    try:
        cid = int(args[1].strip())
        await db.del_fsub(cid)
        await update_fsub_chats_var()
        await message.reply(f"<blockquote><b>✅ Removed FSub: {cid}</b></blockquote>")
    except Exception as e:
        await message.reply(f"<blockquote><b>Error: {e}</b></blockquote>")
...
async def list_fsubs_cmd(client, message: Message):
    try:
        fsubs = await db.get_fsub_chats()
        if not fsubs:
            return await message.reply("<blockquote><b>No FSub channels set.</b></blockquote>")
        text = "<blockquote><b>📋 FSub Channels:</b>\n\n"
        for cid in fsubs:
            try:
                chat = await bot.get_chat(int(cid))
                text += f"• {chat.title} — <code>{cid}</code>\n"
            except Exception:
